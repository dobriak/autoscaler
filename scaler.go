	...
	if !sig.Scale.down && !sig.Scale.up {
		...
		if sig.Scale.up {
			if mApp.App.Instances < a.MaxInstances {
				...
				log.Infof("%s reached max instances %d", a.AppID, a.MaxInstances)
			}
		}
		if sig.Scale.down {
			if mApp.App.Instances > a.MinInstances {
				...
				log.Infof("%s reached min instances %d", a.AppID, a.MinInstances)
			}
		}
	}
...
func (a *App) doScale(mApp MarathonApp, instances int) {
	...
	log.Infof("Scaling %s to %d instances", a.AppID, target)
